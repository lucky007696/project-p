...
The repository is structured to seamlessly preprocess human body imagery and deploy an advanced Try-on Pipeline:

- **`src/`:** Contains the primary virtual try-on pipelines and modified U-Net components (`unet_hacked_tryon`, `unet_hacked_garmnet`).
- **`gradio_demo/`:** Features an interactive local web UI (`app.py`) for visually evaluating single-image try-on processing.
- **`preprocess/`:** Utility modules housing `humanparsing` and `openpose` models to pre-compute agnostic spatial masks and human limb coordinates.
- **`ckpt/` & `configs/`:** Directories intended to store downloaded densepose, parsing checkpoints, and `.yaml` config files mapping HuggingFace models.
